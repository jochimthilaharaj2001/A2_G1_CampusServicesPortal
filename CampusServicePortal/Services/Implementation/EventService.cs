...
public class EventService(ApplicationDbContext context, INotificationQueue notifications) : IEventService
{
    ...
    public async Task<EventDto> SaveEventAsync(int? id, SaveEventDto dto)
    {
        ...
        if (id.HasValue)
        {
            var registeredUsers = await context.EventRegistrations
                .Where(registration => registration.EventId == item.EventId && registration.Status == "Confirmed")
        ...
        return await EventQuery(true).FirstAsync(e => e.EventId == item.EventId);
    }

    public async Task DeactivateEventAsync(int id)
    {
        var item = await context.Events.FindAsync(id) ?? throw new KeyNotFoundException("Event not found.");
        item.IsActive = false;
        item.UpdatedAt = DateTime.UtcNow;

        var registeredUsers = await context.EventRegistrations
            .Where(registration => registration.EventId == id && registration.Status == "Confirmed")
            .Select(registration => registration.UserId)
            .ToListAsync();
    ...
    public async Task<EventRegistrationDto> RegisterAsync(int userId, CreateEventRegistrationDto dto)
    {
        ...
        var registration = new EventRegistration { EventId = item.EventId, UserId = userId, EventSeatId = seat?.EventSeatId };
        ...
        await context.SaveChangesAsync();
        return await RegistrationQuery().FirstAsync(item => item.EventRegistrationId == registration.EventRegistrationId);
    }
    ...
    public async Task CancelRegistrationAsync(int id, int userId, bool isAdmin)
    {
        var item = await context.EventRegistrations.FindAsync(id) ?? throw new KeyNotFoundException("Event registration not found.");
        if (!isAdmin && item.UserId != userId) throw new UnauthorizedAccessException();
        if (item.Status != "Confirmed") throw new InvalidOperationException("This registration is already cancelled.");
