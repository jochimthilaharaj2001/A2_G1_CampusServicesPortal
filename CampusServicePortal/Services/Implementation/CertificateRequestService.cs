...
public class CertificateRequestService(ApplicationDbContext context, INotificationQueue notifications) : ICertificateRequestService
{
    ...
    public async Task<IReadOnlyList<CertificateRequestDto>> GetForUserAsync(int userId) =>
        await Query().Where(r => r.UserId == userId).OrderByDescending(r => r.RequestedAt).ToListAsync();

    public async Task<IReadOnlyList<CertificateRequestDto>> GetAllAsync(string? status)
    {
        var query = Query();
    ...
    public async Task<CertificateRequestDto> UpdateStatusAsync(int id, UpdateCertificateRequestStatusDto dto)
    {
        var request = await context.CertificateRequests.FindAsync(id)
        ...
        if (!string.Equals(previousStatus, dto.Status, StringComparison.Ordinal))
        {
            await notifications.QueueForUserAsync(
                ...
                $"Your certificate request status is now {dto.Status}.");
        }

        await context.SaveChangesAsync();
        return await Query().FirstAsync(item => item.CertificateRequestId == id);
    }
